...
class JourneyTest {
    ...
    def void "given a Journey, it should have a name and contains appropriate steps"(){
        ...
        Journey journey = new Journey([new Edge(GoToLoginPage), new Edge(EnterPassword)], TestClass)
        journey.init(1)
        assert journey.name == 'Test[1]  JourneyTest$GoToLoginPage  JourneyTest$EnterPassword '
        assert journey.trail == [new Edge(GoToLoginPage), new Edge(EnterPassword)]
    ...
    def void "given a Journey with descriptions, the journey could contain a nice description"(){
        ...
        Journey journey = new Journey([new Edge(GoToForgotPasswordPage), new Edge(EnterEmailAddress)], TestClass)
        journey.init(1)
